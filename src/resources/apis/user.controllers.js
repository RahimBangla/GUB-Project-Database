 ...
 * @route GET /notes/:id
 */
export const getSingleVehicle = tryCatchWrapper(async function (
  req,
  res,
  next
) {
...
export const Login = tryCatchWrapper(async function (req, res, next) {
  ...
  return res
    .status(200)
    .json({ data: rows[0], message: "Login successfully", status: true });
});
...
export const SingUp = tryCatchWrapper(async function (req, res, next) {
  ...
  return res
    .status(201)
    .json({ message: "Registration Successfull!", status: true });
});
/**
...
export const updateVehicle = tryCatchWrapper(async function (req, res, next) {
  ...
  let sql =
    "UPDATE vehicle SET vehicle_type = ?, mechanical_problem = ?, license_number = ? WHERE vehicle_id = ?";
